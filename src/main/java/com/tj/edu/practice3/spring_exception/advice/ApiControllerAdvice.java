...
public class ApiControllerAdvice {
    ...
    public ResponseEntity nullPointerException(NullPointerException npe) {
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("   내부에서 객체를 생성하지 않았습니다.");

    }

    @ExceptionHandler(value = MissingServletRequestParameterException.class)
    public ResponseEntity missingServletRequestParameterException(MissingServletRequestParameterException msrpe) {
        System.out.println("필수 값을 보내주세요.");
        System.out.println(msrpe.getBody());

        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("(advice) age value : Null");
    }
}
